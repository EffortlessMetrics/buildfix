        ...
        for manifest in Self::manifest_paths_from_triggers(&matched) {
            ...
            let (safety, params_required, rust_version_value) = match &rust_version {
                Some(rv) => {
                    ...
                    (safety, vec![], serde_json::Value::String(rv.clone()))
                }
                None => (
                    SafetyClass::Unsafe,
...
/// Determines the safety class for MSRV normalization based on evidence.
///
/// # Safety Promotion Logic
///
/// An operation is promoted to [`SafetyClass::Safe`] when ALL of the following conditions are met:
/// - **Workspace canonical exists**: The workspace has a defined rust-version in `[workspace.package]`
/// - **Full consensus**: All workspace crates agree on the rust-version value

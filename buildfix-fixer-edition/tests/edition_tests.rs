//! Integration tests for buildfix-fixer-edition
//!
//! These tests complement the inline tests in src/edition.rs

use buildfix_fixer_api::{Fixer, PlanContext, PlannerConfig, ReceiptSet, RepoView};
use buildfix_fixer_edition::EditionUpgradeFixer;
...
/// Create a receipt set with an edition finding
fn receipt_set_with_edition_finding(sensor: &str, check_id: &str, path: &str) -> ReceiptSet {
...
fn fixer_meta_returns_correct_fix_key() {
    let fixer = EditionUpgradeFixer;
    let meta = fixer.meta();
    assert_eq!(meta.fix_key, "cargo.normalize_edition");
}
...
fn fixer_meta_returns_correct_description() {
    ...
    assert!(meta.description.contains("edition"));
}
...
#[test]
fn plan_returns_empty_when_edition_already_matches() {
    let fixer = EditionUpgradeFixer;
    let repo = MockRepo::new(&[
        (
            "Cargo.toml",
...
edition = "2021""#,
    ...
    let ops = fixer
        .plan(
            &ctx,
            &repo,
            &receipt_set_with_edition_finding(
                "builddiag",
                "edition.consistent",
                "crates/a/Cargo.toml",
            ),
...
#[test]
fn plan_generates_guarded_op_when_edition_mismatch() {
    let fixer = EditionUpgradeFixer;
    let repo = MockRepo::new(&[
        (
            "Cargo.toml",
...
edition = "2018""#,
    ...
    assert_eq!(op.params_required, vec!["edition"]);
}
...
#[test]
fn plan_responds_to_rust_edition_consistent_check_id() {
    let fixer = EditionUpgradeFixer;
    let repo = MockRepo::new(&[
        (
            "Cargo.toml",
...
#[test]
fn plan_responds_to_cargo_edition_consistent_check_id() {
    let fixer = EditionUpgradeFixer;
    let repo = MockRepo::new(&[
        (
            "Cargo.toml",
...
// ============================================================================
// Workspace Canonical Edition Resolution Tests
// ============================================================================

#[test]
fn plan_uses_workspace_package_edition_as_canonical() {
    let fixer = EditionUpgradeFixer;
    let repo = MockRepo::new(&[
        (
            "Cargo.toml",
...
edition = "2015""#,
    ...
    match &ops[0].kind {
        OpKind::TomlTransform { args, .. } => {
            // Should use workspace.package.edition (2021), not package.edition (2018)
            assert_eq!(args.as_ref().unwrap()["edition"], "2021");
        }
...
#[test]
fn plan_falls_back_to_package_edition_when_no_workspace_package() {
    let fixer = EditionUpgradeFixer;
    let repo = MockRepo::new(&[
        (
            "Cargo.toml",
...
edition = "2015""#,
    ...
    match &ops[0].kind {
        OpKind::TomlTransform { args, .. } => {
            // Should fall back to package.edition (2018)
            assert_eq!(args.as_ref().unwrap()["edition"], "2018");
        }
        _ => panic!("expected TomlTransform"),
    }
}

...
cargo run -p buildfix -- apply --apply               # Apply safe fixes
cargo run -p buildfix -- apply --apply --allow-guarded  # Include guarded fixes
```
...
- **buildfix-types**: Shared DTOs and schemas (`BuildfixPlan`, `Operation`, `SafetyClass`)
...
3. Each fixer emits `PlannedFix` operations with safety class
...
- `Fixer` trait: `plan(ctx, repo, receipts) -> Vec<PlannedFix>`
